window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CodePanel(props) {
      ...
      const BATCH_SIZE = 80;
      const [visibleCount, setVisibleCount] = React.useState(Math.min(rows.length, BATCH_SIZE));
      ...
      React.useEffect(() => {
        ...
        const observer = new IntersectionObserver((entries) => {
          ...
          if (entry && entry.isIntersecting) {
            setVisibleCount((prev) => Math.min(rows.length, prev + BATCH_SIZE));
          }
      ...
      if (rows.length) {
        ...
        if (visibleCount < rows.length) {
          children.push(React.createElement('div', { className: 'cdv-lazy-sentinel', key: 'sentinel', ref: sentinelRef }));
        }
      } else {
    ...
    function EditWriteView(props) {
      ...
      const [open, setOpen] = React.useState(Boolean(block && block.isError));
...
.cdv-lazy-sentinel { position: relative; height: 1px; margin: 0; opacity: 0; pointer-events: none; }
`;
