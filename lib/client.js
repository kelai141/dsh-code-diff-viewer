window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CodePanel(props) {
      const { title, isNew, rows, group, empty, offset, codesRef, onCodesScroll } = props;
      const children = [];
      if (rows.length) {
      ...
      return React.createElement('div', { className: 'cdv-col' + (isNew ? ' cdv-col-new' : '') },
        ...
        React.createElement('div', { className: 'cdv-codes', ref: codesRef, onScroll: onCodesScroll }, children)
      );
    }
              ...
              if (both) {
                cols = [
                  React.createElement(CodePanel, {
                    key: 'a', title: '修改前', isNew: false, rows: h.left, group: h.group, empty: '（无内容）', offset,
                  ...
                  React.createElement(CodePanel, {
                    key: 'b', title: '修改后', isNew: true, rows: h.right, group: h.group, empty: '（无内容）', offset,
